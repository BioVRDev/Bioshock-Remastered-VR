...
### `XRSession.cpp` (1443) — OpenXR session, swapchains, submission
...
### `CameraHook.cpp` (2615) — the camera seam ⚠️ largest file
...
### `DrawHook.cpp` (2174) — draw classification and HUD capture ⚠️ second largest
...
### `InputHook.cpp` (1362) — OpenXR actions → synthetic XInput
...
### `GameState.cpp` (1178) — the game's own UI/context state
...
### `HandsProbe.cpp` (1732) — pawn/hands/weapon discovery
...
### `ArmHide.cpp` (507) — sleeve and inactive-hand suppression
...
### `Swing.cpp` (232) — physical wrench gesture
...
### `EngineExec.cpp` (279) — the Unreal `Exec` seam
...
### `GameIni.cpp` (218) — game config synchronisation
...
### `Keybinds.cpp` (219) — rebindable keys ⚠️ **dead code**
