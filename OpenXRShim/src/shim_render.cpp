// ============================================================================
//  shim_render.cpp -- per-eye compositor: OpenXR layers -> OpenVR Submit.
//
...
bool Render_Init()
{
    ...
    for (int p = 0; p < 2; ++p)
        for (int e = 0; e < 2; ++e)
        {
            if (FAILED(dev->CreateTexture2D(&td, nullptr, &g_eyeTex[p][e])) ||
                FAILED(dev->CreateRenderTargetView(g_eyeTex[p][e], nullptr, &g_eyeRtv[p][e])))
            {
                SLOG("!!! render: eye RT create failed");
                return false;
            }
        }
